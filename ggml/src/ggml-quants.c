    ...
    for (int i = 0; i < nb; i++) {
        for (int s = 0; s < n_sub; s++) {
            ...
            for (int j = 0; j < qk_sub/2; ++j) {
                const uint8_t x0 = ggml_mxfp_best_index_e2m1(xb[0        + j], d);
                const uint8_t x1 = ggml_mxfp_best_index_e2m1(xb[qk_sub/2 + j], d);
