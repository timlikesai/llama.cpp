...
static float mse_error_fp8_e5m2(float val, float inv_scale, float scale) {
    const float recon = fp8_e5m2_to_float(float_to_fp8_e5m2_rn(val * inv_scale)) * scale;
    const float err = val - recon;
    return err * err;
}
...
static float mse_error_fp6_e3m2(float val, float inv_scale, float scale) {
    const float recon = fp6_e3m2_to_float(float_to_fp6_e3m2_rn(val * inv_scale)) * scale;
    const float err = val - recon;
    return err * err;
}
...
void quantize_row_mxfp4_soa(const float * GGML_RESTRICT x, void * GGML_RESTRICT dst, int64_t k) {
    ...
    char * row = (char *)dst;
    char * qs_base  = row;
    char * e8m0_base = row + nb * MXFP4_SOA_QS_PER_BLOCK;
    ...
    for (int i = 0; i < nb; i++) {
        ...
        uint8_t * qs = (uint8_t *)(qs_base + i * MXFP4_SOA_QS_PER_BLOCK);
...
static void quantize_row_mxfp8_soa_impl(const float * GGML_RESTRICT x, void * GGML_RESTRICT dst,
    ...
    char * row = (char *)dst;
    char * qs_base   = row;
    char * e8m0_base = row + nb * MXFP8_SOA_QS_PER_BLOCK;
    ...
    for (int i = 0; i < nb; i++) {
        ...
        uint8_t * qs = (uint8_t *)(qs_base + i * MXFP8_SOA_QS_PER_BLOCK);
        for (int j = 0; j < QK_MXFP8; ++j) {
...
static void dequantize_row_mxfp8_soa_impl(const void * GGML_RESTRICT src, float * GGML_RESTRICT y,
    ...
    for (int i = 0; i < nb; i++) {
        ...
        const uint8_t * qs = (const uint8_t *)(qs_base + i * MXFP8_SOA_QS_PER_BLOCK);
        for (int j = 0; j < QK_MXFP8; ++j) {
...
static void quantize_row_mxfp6_soa_impl(const float * GGML_RESTRICT x, void * GGML_RESTRICT dst,
    ...
    char * row = (char *)dst;
    char * qs_base   = row;
    char * e8m0_base = row + nb * MXFP6_SOA_QS_PER_BLOCK;
    ...
    for (int i = 0; i < nb; i++) {
        ...
        uint8_t * qs = (uint8_t *)(qs_base + i * MXFP6_SOA_QS_PER_BLOCK);
        for (int j = 0; j < QK_MXFP6; j += 4) {
            uint8_t vals[4];
            for (int jj = 0; jj < 4; jj++) {
                vals[jj] = traits->to_elem(x[i*QK_MXFP6 + j + jj] * inv_d);
...
static void dequantize_row_mxfp6_soa_impl(const void * GGML_RESTRICT src, float * GGML_RESTRICT y,
    ...
    for (int i = 0; i < nb; i++) {
        const float d = GGML_E8M0_TO_FP32((uint8_t)e8m0_base[i]);
        const uint8_t * qs = (const uint8_t *)(qs_base + i * MXFP6_SOA_QS_PER_BLOCK);
        for (int j = 0; j < QK_MXFP6; j += 4) {
            uint8_t vals[4];
...
void quantize_row_mxfp8_e5m2_soa(const float * GGML_RESTRICT x, void * GGML_RESTRICT dst, int64_t k) {
    quantize_row_mxfp8_soa_impl(x, dst, k, &mxfp8_e5m2_traits);
}
void dequantize_row_mxfp8_e5m2_soa(const void * GGML_RESTRICT src, float * GGML_RESTRICT y, int64_t k) {
    dequantize_row_mxfp8_soa_impl(src, y, k, &mxfp8_e5m2_traits);
}
void quantize_row_mxfp6_e2m3_soa(const float * GGML_RESTRICT x, void * GGML_RESTRICT dst, int64_t k) {
    quantize_row_mxfp6_soa_impl(x, dst, k, &mxfp6_e2m3_traits);
}
void dequantize_row_mxfp6_e2m3_soa(const void * GGML_RESTRICT src, float * GGML_RESTRICT y, int64_t k) {
...
void quantize_row_mxfp6_e3m2_soa(const float * GGML_RESTRICT x, void * GGML_RESTRICT dst, int64_t k) {
    quantize_row_mxfp6_soa_impl(x, dst, k, &mxfp6_e3m2_traits);
}
void dequantize_row_mxfp6_e3m2_soa(const void * GGML_RESTRICT src, float * GGML_RESTRICT y, int64_t k) {
    dequantize_row_mxfp6_soa_impl(src, y, k, &mxfp6_e3m2_traits);
}
