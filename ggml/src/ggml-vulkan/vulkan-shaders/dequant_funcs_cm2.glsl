...
// -----------------------------------------------------------------------
// MXFP SoA (Struct-of-Arrays) decode functions for cooperative matrix loads.
// SoA layout per row: [qs_block0|qs_block1|...|qs_blockN][e8m0_0|e8m0_1|...|e8m0_N]
// The buffer reference `bl` points to byte (blockCoords[0] * stride + blockCoords[1]) * block_byte_size
// within the raw buffer. We compute SoA offsets relative to bl using blockCoords[1] (block within row)
// and mxfp_active_stride (set by caller before coopMatLoadTensorNV).
// -----------------------------------------------------------------------
// mxfp_active_stride: declared in flash_attn_base.glsl alongside k_stride/v_stride.
// Set by flash_attn_cm2.comp to k_stride or v_stride before each coopMatLoadTensorNV call.

// --- MXFP4 E2M1 SoA decode (block_byte_size=17, qs_per_block=16) ---
// Buffer reference name MUST match: dequantFunc{NAME} → decodeBuf{NAME}
#if defined(DATA_A_MXFP4) || defined(MXFP_ALL_DEQUANT)
layout(buffer_reference, std430, buffer_reference_align = 1) readonly buffer decodeBufMXFP4 {
    uint8_t data;
};
float16_t dequantFuncMXFP4(const in decodeBufMXFP4 bl, const in uint blockCoords[2], const in uint coordInBlock[2]) {
    return float16_t(1.0); // DEBUG: return constant 1.0 to verify decode function is called
}
#endif

// --- MXFP8 E4M3 SoA decode (block_byte_size=33, qs_per_block=32) ---
#if defined(DATA_A_MXFP8_E4M3) || defined(MXFP_ALL_DEQUANT)
layout(buffer_reference, std430, buffer_reference_align = 1) readonly buffer decodeBufMXFP8_E4M3 {
    uint8_t data;
};
float16_t dequantFuncMXFP8_E4M3(const in decodeBufMXFP8_E4M3 bl, const in uint blockCoords[2], const in uint coordInBlock[2]) {
    const uint bwr  = blockCoords[1];
    const uint elem = coordInBlock[1];

    const int e8m0_rel = 32 * int(mxfp_active_stride - bwr);
    const float scale = e8m0_to_fp32(uint8_t(decodeBufMXFP8_E4M3(uint64_t(bl) + e8m0_rel).data));

    const int qs_rel = -int(bwr) + int(elem);
    const uint qs_byte = uint(decodeBufMXFP8_E4M3(uint64_t(bl) + qs_rel).data);

    return float16_t(fp8_e4m3_to_float(qs_byte) * scale);
}
#endif

// --- MXFP8 E5M2 SoA decode (block_byte_size=33, qs_per_block=32) ---
#if defined(DATA_A_MXFP8_E5M2) || defined(MXFP_ALL_DEQUANT)
layout(buffer_reference, std430, buffer_reference_align = 1) readonly buffer decodeBufMXFP8_E5M2 {
    uint8_t data;
};
float16_t dequantFuncMXFP8_E5M2(const in decodeBufMXFP8_E5M2 bl, const in uint blockCoords[2], const in uint coordInBlock[2]) {
    const uint bwr  = blockCoords[1];
    ...
    const int e8m0_rel = 32 * int(mxfp_active_stride - bwr);
    const float scale = e8m0_to_fp32(uint8_t(decodeBufMXFP8_E5M2(uint64_t(bl) + e8m0_rel).data));

    const int qs_rel = -int(bwr) + int(elem);
    const uint qs_byte = uint(decodeBufMXFP8_E5M2(uint64_t(bl) + qs_rel).data);

    return float16_t(fp8_e5m2_to_float(qs_byte) * scale);
}
#endif

// --- MXFP6 E2M3 SoA decode (block_byte_size=25, qs_per_block=24) ---
#if defined(DATA_A_MXFP6_E2M3) || defined(MXFP_ALL_DEQUANT)
layout(buffer_reference, std430, buffer_reference_align = 1) readonly buffer decodeBufMXFP6_E2M3 {
    uint8_t data;
};
float16_t dequantFuncMXFP6_E2M3(const in decodeBufMXFP6_E2M3 bl, const in uint blockCoords[2], const in uint coordInBlock[2]) {
    const uint bwr  = blockCoords[1];
    ...
    // bl at bwr*25. E8M0 at stride*24 + bwr. Rel = 24*(stride - bwr)
    const int e8m0_rel = 24 * int(mxfp_active_stride - bwr);
    const float scale = e8m0_to_fp32(uint8_t(decodeBufMXFP6_E2M3(uint64_t(bl) + e8m0_rel).data));

    // QS: tight 6-bit packing, 4 elements per 3 bytes. bl at bwr*25, QS at bwr*24. Offset = -bwr.
    const uint group = elem / 4u;
    const uint within = elem % 4u;
    const int qs_base_rel = -int(bwr) + int(group * 3u);
    const uint b0 = uint(decodeBufMXFP6_E2M3(uint64_t(bl) + qs_base_rel).data);
    const uint b1 = uint(decodeBufMXFP6_E2M3(uint64_t(bl) + qs_base_rel + 1).data);
    const uint b2 = uint(decodeBufMXFP6_E2M3(uint64_t(bl) + qs_base_rel + 2).data);
    const uint packed = b0 | (b1 << 8) | (b2 << 16);
    const uint val = (packed >> (within * 6u)) & 0x3Fu;

    return float16_t(fp6_e2m3_to_float(val) * scale);
}
#endif

// --- MXFP6 E3M2 SoA decode (block_byte_size=25, qs_per_block=24) ---
#if defined(DATA_A_MXFP6_E3M2) || defined(MXFP_ALL_DEQUANT)
layout(buffer_reference, std430, buffer_reference_align = 1) readonly buffer decodeBufMXFP6_E3M2 {
    uint8_t data;
};
float16_t dequantFuncMXFP6_E3M2(const in decodeBufMXFP6_E3M2 bl, const in uint blockCoords[2], const in uint coordInBlock[2]) {
    const uint bwr  = blockCoords[1];
    ...
    const int e8m0_rel = 24 * int(mxfp_active_stride - bwr);
    const float scale = e8m0_to_fp32(uint8_t(decodeBufMXFP6_E3M2(uint64_t(bl) + e8m0_rel).data));

    const uint group = elem / 4u;
    const uint within = elem % 4u;
    const int qs_base_rel = -int(bwr) + int(group * 3u);
    const uint b0 = uint(decodeBufMXFP6_E3M2(uint64_t(bl) + qs_base_rel).data);
    const uint b1 = uint(decodeBufMXFP6_E3M2(uint64_t(bl) + qs_base_rel + 1).data);
    const uint b2 = uint(decodeBufMXFP6_E3M2(uint64_t(bl) + qs_base_rel + 2).data);
    const uint packed = b0 | (b1 << 8) | (b2 << 16);
    const uint val = (packed >> (within * 6u)) & 0x3Fu;

    return float16_t(fp6_e3m2_to_float(val) * scale);
}
#endif
