...
// Mixed mxfp block-scaled MMA path for Blackwell GeForce.
// A fragment per lane (t = threadIdx.x%4, g = threadIdx.x/4), see mma.cuh:
//   reg0/reg2: row g,   k = t*8 + {0..3} / {4..7}; reg1/reg3: row g+8, same k.
// B fragment: col g, k = t*8 + {0..7} contiguous.
// type_b: activation format (always e4m3).
template <ggml_type type, int J, bool fallback> static __device__ __forceinline__ void ggml_cuda_mmq_vec_dot_mxfp_a_mma(
        const int * __restrict__ x, const int * __restrict__ y, float * __restrict__ sum, const int k00) {
...
#pragma unroll
            for (int frag = 0; frag < nfrags; ++frag) {
                tile_C C = {};
                mma_block_scaled_mxfp<type>(C, A[n][frag], B[frag], scaleA[n][frag], scaleB[frag]);
#pragma unroll
                for (int l = 0; l < tile_C::ne; ++l) {
                    sum[(j0 / tile_C::J + n) * tile_C::ne + l] += C.x[l];
                }
